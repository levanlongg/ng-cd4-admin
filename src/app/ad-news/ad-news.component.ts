import { Component, OnInit, ViewChild } from '@angular/core';
import { ModalDirective } from 'ngx-bootstrap/modal';
import { AdNewsService } from '../Services/ad-news.service';
import * as ClassicEditor from '@ckeditor/ckeditor5-build-classic';

@Component({
  selector: 'app-ad-news',
  templateUrl: './ad-news.component.html',
  styleUrls: ['./ad-news.component.css']
})
export class AdNewsComponent implements OnInit {

  [x: string]: any;

  @ViewChild('editAndADD', { static: false }) editAndADD: ModalDirective;
  @ViewChild('editAndADD1', { static: false }) editAndADD1: ModalDirective;


  // constructor(private News: AdNewsService) {}

  public progress: number;
  public message: string;
  public Editor = ClassicEditor;
  public entity: any;
  public entity1: any;
  public filterTerm: string;
  public items: any[];
  public id: string;
  public checkedid: any;
  public keyword: string;

  constructor(private News: AdNewsService) {
    // this.items= new Array<any>();
  }

  ngOnInit(): void {
    // this.News.getList().subscribe((res: any)=>{

    //   this.items = res;
    //   console.log(this.items);
    // });
    this.loadData();
  }

  loadData() {
    this.News.getList().subscribe((res: any)=>{

    this.items = res;
    console.log(this.items);
    });
  }

  showAdd() {
    this.entity = {};
    this.checkedid = 0;
    this.editAndADD.show();
  }
  showEdit(id: any) {
    debugger;
    this.checkedid = 1;
    this.News.GetSingle(id).subscribe((res) => {
      this.entity = res;
    });
    this.editAndADD.show();
  }
  showDetail(id: any) {
    debugger;
    this.checkedid = 1;
    this.News.GetSingle(id).subscribe((res) => {
      this.entity1 = res;
    });
    this.editAndADD1.show();
  }
  SaveForm(values: any) {
    if (this.checkedid == 0) {
      this.News.postItme(values).subscribe((res) => {
        if (res) {
          alert('Do you want add this item?');
          this.loadData();
          this.editAndADD.hide();
        }
      });
    } else {
      this.id = values.id;
      console.log(values.id);
      this.News.editItem(this.id, values).subscribe((res) => {
        alert('Are you sure edit this item?');
        this.loadData();
        this.editAndADD.hide();
      });
    }
  }
  deleteShow(id: string) {
    if (confirm('Are you sure delete this item?')) {
      this.News.deleteItem(id).subscribe((res) => {
        this.loadData();
      });
    }
  }

  Search() {
    this.News.Search(this.keyword).subscribe((response: any) => {
        this. items= response;
        console.log(response);
    }, error => {
        console.log(error);
    });
  }
}

